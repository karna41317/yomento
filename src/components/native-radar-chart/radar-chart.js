  ...
  updateChart () {
    ...
    if (options) {
      this.chart_instance.options = Chart.helpers.configMerge(
        this.chart_instance.options, options)
    }

    // Pipe datasets to chart instance datasets enabling
    // seamless transitions
    let currentDatasets = (this.chart_instance.config.data &&
    ...
    for (let idx = currentDatasets.length - 1; idx >= 0; idx -= 1) {
      ...
      if (nextDatasetKeys.indexOf(currentDatasetKey) === -1) {
        ...
        const retainedDataset = find(nextDatasets,
...
export default class NativeRadarComponent extends React.Component {
  render () {
    return (
      <ChartComponent
        {...this.props}

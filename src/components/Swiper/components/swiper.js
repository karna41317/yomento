...
export default class Swiper extends Component {
  ...
  componentWillReceiveProps (nextProps) {
    if (!nextProps.autoplay && this.autoplayTimer) clearTimeout(
      this.autoplayTimer)
    this.setState(
  ...
  componentWillUpdate (nextProps, nextState) {
    ...
    if (this.state.index !== nextState.index) this.props.onIndexChanged(
      nextState.index)
  }
  ...
  initState (props, updateIndex = false) {
    ...
    if (state.total === initState.total && !updateIndex) {
      ...
      initState.index = initState.total > 1 ? Math.min(
        props.index, initState.total - 1) : 0
    }
  ...
  onLayout = (event) => {
    ...
    if (!this.state.offset || width !== this.state.width ||
